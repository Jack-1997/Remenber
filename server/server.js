...
// 跨域代理
// app.get('/dimDicGb/getByDmbh?dmbh=3607', function(req, res) {
//     proxy(req, res, function(req, res, data) {
//         console.log(data);
...
function proxy(req, res, callback) {
   ...
   /// var backend = process.env.BACKEND || "http://192.168.0.74:8090";
    //http://192.168.0.94:8090/huanhuan
    var backend = process.env.BACKEND || "http://192.168.0.222:80";
    var path = req.url.replace(/^\/?api/, "");
    console.log(backend + req.path);
    debugger;
    if (req.method === 'GET') {
        
        request.get(backend + path).on("response", function(response) {
            var data = "";

            response.on("data", function(chunk) {
                data += chunk;
                //console.log("134行"+ ":" +data )
...
app.listen(667);
console.log("服务器已启动  =>  localhost:667");